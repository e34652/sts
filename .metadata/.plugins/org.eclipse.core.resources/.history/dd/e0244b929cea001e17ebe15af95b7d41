...
<body>
	...
	<c:if test="${eListByDeptno eq null}">
		<table>
			<tr>
				<th>사번</th>
				<th>이름</th>
			...
			<c:forEach var="eList" items="${EmpList}">
				...
				<tr>
					...
					<td>
						<button
							onclick="window.location.href='pressEdit?empno=${eList.empno}'">수정</button>
						<button
							onclick="window.location.href='pressDelete?empno=${eList.empno}'">삭제</button>
					</td>
				</tr>
			</c:forEach>
		</table>
